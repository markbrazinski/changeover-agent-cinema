...
test.describe('Padded Absolute-Time Filming Sequence Qualification', () => {
  test('Full end-to-end filming run (F key) executes padded 144.7s absolute timeline', async ({ page }) => {
    ...
    // 54.0s: Hard cut to Refusal baseline (08a_refusal_baseline)
    ...
    console.log(`[E2E TIMING] Cut 1 to Refusal occurred at: ${cut1Time.toFixed(2)}s (Target: 54.0s)`);
    expect(cut1Time).toBeGreaterThanOrEqual(52.0);
    expect(cut1Time).toBeLessThanOrEqual(56.5);

    // 72.0s: Hard cut to Contention baseline (09a_contention_baseline)
    await expect(page.getByTestId('facility-view')).toBeVisible({ timeout: 25000 });
    const cut2Time = (Date.now() - startTime) / 1000;
    console.log(`[E2E TIMING] Cut 2 to Contention occurred at: ${cut2Time.toFixed(2)}s (Target: 72.0s)`);
    ...
    // Wait for full 126.0s timeline completion
    await page.waitForTimeout(20000);

    const totalDuration = (Date.now() - startTime) / 1000;
    console.log(`[E2E TIMING] Total filming run duration: ${totalDuration.toFixed(2)}s (Target: 126.0s)`);
    expect(totalDuration).toBeGreaterThanOrEqual(123.0);
    expect(totalDuration).toBeLessThanOrEqual(129.0);
  });
